...
type GameWeek = { week:number; status:string; deadlineUtc?:string; sleeperStatus:string; team?:SyncedTeam; hand:DealtCard[]; selections:SavedSelection[]; miniBattlePlayerIds?:string[]; publicSelections?:{rosterId:number;selections:SavedSelection[]}[]; canDraw?:boolean; cardsNeeded?:number; teams?:SyncedTeam[]; matchups:SyncedMatchup[]; chaosScores?:ChaosScore[] }
type CardUsageReport = { totalPlays:number; uniqueCards:number; summary:{cardId:string;cardName:string;category:string;plays:number;managers:number;lastPlayedWeek:number}[]; plays:{week:number;status:string;selectedAtUtc:string;rosterId:number;manager:string;teamName:string;cardId:string;cardName:string;category:string;targetTeam:string;targetPlayer:string;targetSlot:string}[] }

const cardTargetOptions = ['Your team','Opponent team','Your starter','Opponent starter','Your QB','Opponent QB','Your RB','Opponent RB','Your WR','Opponent WR','Your TE','Opponent TE','Your DEF','Opponent DEF','Your RB/WR/TE','Any starting position','Weekly opponent WR','Weekly opponent RB','Weekly opponent TE','Opponent card','Dynamic target']
...
function CardCreator({ initialCard, onSave, onCancel }: { initialCard: UploadedCard | null; onSave: (card: UploadedCard) => Promise<void> | void; onCancel: () => void }) {
  ...
  const knownRule = cardDataIdeas.find(card=>card.name.toLowerCase()===name.trim().toLowerCase())

  const changeName = (nextName:string) => {
    setName(nextName)
    if(initialCard)return
    const preset=cardDataIdeas.find(card=>card.name.toLowerCase()===nextName.trim().toLowerCase())
    if(!preset)return
    setCategory(preset.category as Category);setDescription(preset.officialDescription);setTarget(preset.target);setEffectType(preset.effectType);setAmount(preset.amount);setCopies(preset.copies);setSpecial(Boolean(preset.isSpecial))
  }
  ...
  return <main className="admin-page" onPaste={handleArtworkPaste}><div className="admin-heading"><div><div className="eyebrow">SHARED COMMISSIONER WORKSPACE</div><h1>{initialCard ? 'Edit Card Draft' : 'Card Creator'}</h1><p>Save artwork and ideas now. Finish the engine rules together before approving the card for the deck.</p></div><button className="secondary" onClick={onCancel}>VIEW CARD LIBRARY →</button></div>
    <div className="creator-layout"><section className="creator-form">
      <div className="form-section"><h3>01 · CARD IDENTITY & SEARCH DATA</h3><div className="form-grid"><label className="wide">CARD NAME<input value={name} onChange={e=>changeName(e.target.value)} placeholder="e.g. Crushing Blow" /></label><label>CATEGORY<select value={category} onChange={e=>{const next=e.target.value as Category;setCategory(next);if(next==='UNIQUE')setEffectType('Specialty rule')}}><option>ATTACK</option><option>BOOST</option><option>UNIQUE</option></select></label><label>RARITY<select value={rarity} onChange={e=>setRarity(e.target.value)}><option>Common</option><option>Uncommon</option><option>Rare</option><option>Legendary</option></select></label><label className="wide">RULES SUMMARY FOR THE GAME LOG<textarea value={description} onChange={e=>setDescription(e.target.value)} placeholder="Store the card effect as searchable text. This will not be printed over the artwork." /></label>{knownRule&&<div className="rule-match wide"><b>✓ ENGINE RULE MATCHED</b><span>The saved settings are tied to the automatic “{knownRule.name}” scoring rule. Its description, target, and configured value are shown below for review.</span></div>}</div></div>
...
function CardUsage({leagueId}:{leagueId:string}) {
  const [report,setReport]=useState<CardUsageReport|null>(null)
  const [error,setError]=useState('')
  useEffect(()=>{apiFetch<CardUsageReport>(`/api/leagues/${leagueId}/game/card-usage`).then(setReport).catch(ex=>setError((ex as Error).message))},[leagueId])
  if(error)return <main className="usage-page"><div className="setup-message">{error}</div></main>
  if(!report)return <main className="usage-page"><div className="setup-message">Loading card activity…</div></main>
  return <main className="usage-page"><div className="admin-heading"><div><div className="eyebrow">COMMISSIONER ONLY</div><h1>Card Usage</h1><p>Review what managers actually play so the league can balance favorites and overlooked cards after the season.</p></div><div className="usage-total"><b>{report.totalPlays}</b><span>TOTAL PLAYS</span></div></div>
    <section className="usage-summary"><h2>Most Played Cards</h2>{report.summary.length?<div className="usage-table"><div className="usage-row usage-head"><span>CARD</span><span>TYPE</span><span>PLAYS</span><span>MANAGERS</span><span>LAST USED</span></div>{report.summary.map(card=><div className="usage-row" key={card.cardId}><b>{card.cardName}</b><span>{card.category}</span><strong>{card.plays}</strong><span>{card.managers}</span><span>Week {card.lastPlayedWeek}</span></div>)}</div>:<p className="usage-empty">No cards have been selected yet.</p>}</section>
    <section className="usage-history"><h2>Play History</h2>{report.plays.map((play,index)=><article key={`${play.cardId}-${play.week}-${play.rosterId}-${index}`}><b>WEEK {play.week}</b><span><strong>{play.manager}</strong> · {play.teamName}</span><span className={`usage-category ${play.category.toLowerCase()}`}>{play.cardName}</span><span>{[play.targetPlayer,play.targetTeam,play.targetSlot].filter(Boolean).join(' · ')||'Team-wide effect'}</span><small>{new Date(play.selectedAtUtc).toLocaleString()}</small></article>)}</section>
  </main>
}
...
function LeagueGame({ league }: { league: ChaosLeague }) {
  ...
  const [screen, setScreen] = useState<'home' | 'room' | 'battle' | 'admin' | 'library' | 'permissions' | 'rosters' | 'weekly' | 'usage'>('home')
  ...
  const [miniBattlePlayers,setMiniBattlePlayers]=useState<Record<string,string>>({})
  ...
  const isCommissioner = viewerIsPrimary || cardPermissions.length>0
  ...
  const applySyncedWeek=(week:GameWeek)=>{
    setGameWeek(week)
    const saved:Record<string,string>={};for(const id of week.miniBattlePlayerIds||[]){const player=week.team?.players.find(item=>item.playerId===id);if(player)saved[player.position]=id}setMiniBattlePlayers(saved)
  ...
  const eligibleTargets = useMemo(() => {
    ...
    const positions = target.includes('rb/wr/te')||target.includes('w/r/t') ? ['RB','WR','TE'] : ['QB','RB','WR','TE','FLEX','DEF'].filter(item=>target.includes(item.toLowerCase()))
    let eligible = positions.length ? roster.filter(player=>positions.includes(player.position) || (positions.includes('FLEX') && ['RB','WR','TE'].includes(player.position))) : roster
  ...
  const saveMiniBattle=async()=>{
    const playerIds=['QB','RB','WR','TE'].map(position=>miniBattlePlayers[position]).filter(Boolean)
    if(playerIds.length!==4){setToast('Choose one starting QB, RB, WR, and TE.');return}
    try{await apiFetch(`/api/leagues/${workspaceId}/game/weeks/${currentWeek}/mini-battle`,{method:'PUT',body:JSON.stringify({playerIds})});await refreshGame();setToast('Your four Mini Battle players are saved.')}
    catch(error){setToast((error as Error).message)}
  }
  ...
  const miniBattleActive=weeklyCards.some(card=>card.week===currentWeek&&card.active&&card.name.toLowerCase()==='mini battle')

  if (screen === 'usage' && isCommissioner) return <div className="app-shell"><button className="weekly-back" onClick={()=>setScreen('home')}>← BACK TO MY TEAM</button><CardUsage leagueId={workspaceId}/></div>
  ...
  return <div className="app-shell">
    <button className="weekly-nav-button" onClick={()=>setScreen('weekly')}>★ THIS WEEK: {weeklyCards.find(card=>card.week===currentWeek&&card.active)?.name?.toUpperCase()||'WEEKLY CARD'} · VIEW ALL 17 WEEKS</button>
    ...
    <header><button className="brand" onClick={() => {setScreen('home');setActiveNav('')}}><b>CC</b><span>CHAOS CARDS<small>FANTASY FOOTBALL</small></span></button><nav><button className={screen==='home'?'active':''} onClick={()=>{setScreen('home');setActiveNav('')}}>My Team</button><button className={screen==='room'||screen==='battle'?'active':''} onClick={()=>{setScreen('room');setActiveNav('League Room')}}>League Room</button><button onClick={()=>{setScreen('weekly');setActiveNav('Weekly Cards')}}>Weekly Cards</button><button className={screen==='library'?'active':''} onClick={()=>{setScreen('library');setActiveNav('Card Library')}}>Card Library</button></nav><div className="admin-actions">{viewerIsPrimary&&<button className="admin-link roster-link" onClick={()=>{setScreen('rosters');setActiveNav('')}}>⇄ ROSTERS</button>}{viewerIsPrimary&&<button className="admin-link commissioner-link" onClick={()=>{setScreen('permissions');setActiveNav('')}}>♛ COMMISSIONERS</button>}{isCommissioner&&<button className="admin-link usage-link" onClick={()=>{setScreen('usage');setActiveNav('')}}>▤ CARD USAGE</button>}{canCreateCards&&<button className="admin-link creator-link" onClick={()=>{setScreen('admin');setActiveNav('')}}>⚙ CARD CREATOR</button>}<button className="admin-link signout-link" title={account?.email} onClick={logout}>Sign out</button></div><div className="week"><span>{viewerTeam?.name || account?.name || 'MY TEAM'}</span><b className={revealed?'':'preweek'}>{revealed?'REVEALED':'PRE-WEEK'}</b></div><TeamBadge team={viewerTeam || home} small /></header>
    ...
    {screen === 'home' ? <main className="team-home"><section className="team-home-hero"><div className="eyebrow">SIGNED IN AS {account?.email}</div><div className="team-home-title"><TeamBadge team={viewerTeam || home}/><div><span>YOUR FANTASY TEAM</span><h1>{viewerTeam?.name || 'Roster connection pending'}</h1><p>{viewerTeam ? `${viewerTeam.manager} · ${viewerTeam.record}` : 'Your commissioner must approve and connect your Sleeper roster.'}</p></div></div><button className="primary enter-league" onClick={()=>{setScreen('room');setActiveNav('League Room')}}>ENTER LEAGUE ROOM →</button></section><section className="home-summary"><article><span>WEEK 1 MATCHUP</span><strong>{viewerTeam?.score.toFixed(1) || '—'}</strong><p>Sleeper points</p></article><article><span>YOUR CHAOS SCORE</span><strong>{viewerTeam?.chaos.toFixed(1) || '—'}</strong><p>After card effects</p></article><article><span>CARDS IN HAND</span><strong>{viewerTeam?.hand ?? 0}</strong><p>Ready to inspect and play</p></article><article><span>ROSTER STATUS</span><strong className="status-word">{viewerTeam?'CONNECTED':'PENDING'}</strong><p>{viewerTeam?'FantasyTools account linked':'Waiting for commissioner'}</p></article></section><section className="home-panels"><article><div className="section-title"><h3>YOUR CARDS</h3><span>{cards.length} AVAILABLE</span></div><div className="home-card-fan">{cards.slice(0,3).map(card=><ChaosCard card={card} compact key={card.id}/>)}</div><button onClick={()=>{setScreen('battle');setActiveNav('')}}>VIEW HAND & PLAY CARDS →</button></article><article><div className="section-title"><h3>YOUR STARTING ROSTER</h3><span>WEEK 1</span></div>{lineupFor(viewerTeam?.id || home.id,0).slice(0,5).map(player=><div className="home-player" key={`${player.position}-${player.name}`}><b>{player.position}</b><span>{player.name}<small>{player.nflTeam} · vs {player.opponent}</small></span><strong>{player.points.toFixed(1)}</strong></div>)}<button onClick={()=>{setScreen('battle');setActiveNav('')}}>VIEW FULL MATCHUP →</button></article></section></main> : screen === 'rosters' ? <RosterAssignments teams={teamData} assignments={rosterAssignments} claims={rosterClaims} leagueId={workspaceId} onSave={saveRosterAssignment} onRemove={removeRosterAssignment} onReview={reviewRosterClaim}/> : screen === 'permissions' ? <CommissionerAccess teams={teamData} grants={permissionGrants} onChange={updatePermissionGrants} leagueId={workspaceId}/> : screen === 'admin' && canCreateCards ? <CardCreator initialCard={editingCard} onSave={saveUploadedCard} onCancel={()=>{setEditingCard(null);setScreen('library');setActiveNav('Card Library')}}/> : screen === 'library' ? <SharedCardLibrary uploaded={uploadedCards} onCreate={()=>{if(canCreateCards){setEditingCard(null);setScreen('admin');setActiveNav('')}}} onEdit={card=>{if(canCreateCards){setEditingCard(card);setScreen('admin');setActiveNav('')}}} onStatus={updateCardStatus} onDelete={deleteUploadedCard}/> : screen === 'room' ? <main className="room">
      ...
      {miniBattleActive&&gameWeek?.status==='selection_open'&&<section className="pending-zone mini-battle-picker"><div className="section-title"><h3>MINI BATTLE · CHOOSE YOUR FOUR</h3><span>{Object.keys(miniBattlePlayers).length} / 4 SAVED</span></div><p>Only one starting QB, RB, WR, and TE will count toward your score. If you miss the deadline, the highest projected eligible starter is chosen automatically.</p><div className="mini-battle-groups">{['QB','RB','WR','TE'].map(position=><fieldset key={position}><legend>{position}</legend>{(gameWeek?.team?.players||[]).filter(player=>player.starter&&player.position===position).map(player=><button className={miniBattlePlayers[position]===player.playerId?'selected':''} key={player.playerId} onClick={()=>setMiniBattlePlayers(current=>({...current,[position]:player.playerId}))}><b>{player.name}</b><span>{player.nflTeam} · {player.points.toFixed(1)} PTS</span></button>)}</fieldset>)}</div><button className="primary" onClick={saveMiniBattle}>SAVE MINI BATTLE LINEUP →</button></section>}
